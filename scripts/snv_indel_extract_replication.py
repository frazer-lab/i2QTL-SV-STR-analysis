...
def process_vcf_and_generate_qc_info(fn, out_dir, ipscore_samples, pair_samples, suff = 'auto', chroms = False):
    ...
    #             print header_dict
                ...
                cols_replication = ['CHROM', 'POS', 'ID','NP_VAR', 'NP_CONC', 'NP_DISC', 'NP_MISSING', 'RR']
                indel_info_header = ['CHROM', 'POS', 'ID', 'VARIANT_TYPE', 'BP_CHANGE']
                


                replication_file_header = (cols_replication + ['variant_type', 'bp_change'])
            ...
            if chrom not in ['X', 'Y']:
                ...
                if len(t) > 0:
                    ...
                    if pair_data_gt != False:
                        ...
                        OUT_LINE_REPLICATION_GT = ([chrom, POS, ID] + OUT_PAIR_DATA_GT  + [variant_type, bp_change])
