...
def process_vcf_and_annotate(fn, out_dir, sample_subset, fn_out, exclude = False):
...
#     fn_annotated_vcf = os.path.join(out_dir, fn_out)
    fn_out_vars = os.path.join(out_dir, fn_out)
#     fn_maf = os.path.join(out_dir, 'hipstr_maf_rna.tsv')
    out_vars_file = open(fn_out_vars, 'w')
    r = fn_out_vars.split('.')[-1]
    fn_out_gs_ft = fn_out_vars.replace(".{}".format(r), '_gs_ft.{}'.format(r))
    out_vars_gs_ft_file = open(fn_out_gs_ft, 'w')
...
#     info_fields = 'INFRAME_PGEOM INFRAME_UP INFRAME_DOWN OUTFRAME_PGEOM OUTFRAME_UP OUTFRAME_DOWN BPDIFFS START END AN REFAC AC NSKIP NFILT DP DSNP DSTUTTER DFLANKINDEL'.split()
    ...
    for line in F:
        ...
        if count == header_end-1:
            ...
            header_cols = ['ID', 'CHROM', 'POS', 'CALLER', 'NONREF_AF', 'ALLELES_DIST', 'NNREF', 'SVTYPE', 
                           'FILTER', 'MAF', 'MODE']
            
            out_vars_file.write("\t".join(header_cols) +'\n')
            out_vars_gs_ft_file.write("\t".join(header_cols) +'\n')
            ...
            if exclude:
                ...
                if caller == 'LUMPY':
                    ...
                    data_dict = calculate_alt_allele_freq(gt_dict, sample_subset, chrom)
                ...
                if caller in ['GS', 'GS_LCNV']:
                    ...
                    out_line = [ID, chrom, POS, caller, maf, alleles_dist, num_alt, svtype, FILTER, maf, mc_allele]
                    
                      
                    if caller == 'GS':
                        ft_dict = geno_extracted[geno_fields_dict['FT']]
                        maf, count_mc_allele, mc_allele, num_passing, num_alt, alleles_dist = compute_maf_dosage_gs_ft(cn_dict,ft_dict, sample_subset)
                        out_line_gs_ft = [ID, chrom, POS, caller, maf, alleles_dist, num_alt, svtype, 
                                    FILTER, maf, mc_allele]
                        out_vars_gs_ft_file.write("\t".join(map(str, out_line_gs_ft)) + '\n')
                ...
                if caller == 'MELT':
                    ...
                    data_dict = calculate_alt_allele_freq(gt_dict, sample_subset, chrom)
                    out_line = [ID, chrom, POS, caller, data_dict['NNREF_AF'], data_dict['ALLELES_DIST'], data_dict['NREF'],
                                svtype, FILTER,  data_dict['MAF'], data_dict['MODE_ALLELE']]
                ...
                out_vars_file.write("\t".join(map(str, out_line)) + '\n')
                num_variants_out +=1
    ...
    out_vars_file.close()
    out_vars_gs_ft_file.close()
    print "number of variants processed: {}".format(num_variants)
    print "number of variants outputted: {}".format(num_variants_out)

    d = datetime.datetime.now()
    ts = d.strftime('%D- %H:%M')
    print "Completed Variant Processing: computing allele_frequencies {}".format(ts)
    print 'COMPLETE'
    print 'files generated: {}\n{}'.format(fn_out_vars,  fn_out_gs_ft)
    return
...
def add_arguments_to_parser(parser):
    ...
    parser.add_argument("-id_exclude", "--id_fn", dest="id_fn", metavar='<IDs to exclude in the final output>', help="file of pairs of IDs one per line to keep in the output", required=False, default = False)
...
def run_from_args(args):
    ...
    process_vcf_and_annotate(vcf_fn, out_dir, samples, fn_out, exclude = id_fn)    
