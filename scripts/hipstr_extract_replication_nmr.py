...
def process_vcf_and_generate_qc_info(fn, out_dir, ipscore_samples, unrelated, pair_samples, suff = False):
...
#             print header_dict
            ...
            if suff:
                cols_info_extracted_mod = ['{}_{}'.format(i, suff) for i in cols_info_extracted]
                
                col_name_dict = {i:"{}_{}".format(i, suff) for i in cols_info_extracted}
               
            else:
                col_name_dict = {i:i  for i in cols_info_extracted}
            col_name_dict['PERIOD'] = 'PERIOD'
...
#             num_missing = len(missing_samps)
            ...
            if percent_missing == 1:
                ...
                DP =  info_cols_dict[col_name_dict['DP']]
                DSTUTTER = info_cols_dict[col_name_dict['DSTUTTER']]
                DFLANKINDEL = info_cols_dict[col_name_dict['DFLANKINDEL']]
                PERIOD = info_cols_dict['PERIOD']
                NFILT = info_cols_dict[col_name_dict['NFILT']]
                INFRAME_UP = info_cols_dict[col_name_dict['INFRAME_UP']]
                INFRAME_DOWN  = info_cols_dict[col_name_dict['INFRAME_DOWN']]
                
                try:
                    perc_dp = round((int(DP)/num_passing), 4)
                except:
                    perc_dp = 'NA'
                
                try:
                    perc_dstutter = round((int(DSTUTTER)/int(DP)),4)
                except:
                    perc_dstutter = 'NA'
                try:
                    perc_flankindel = round((int(DFLANKINDEL)/int(DP)), 4)
                except:
                    perc_flankindel = 'NA'
...
def run_from_args(args):
    ...
    process_vcf_and_generate_qc_info(vcf_fn, out_dir, samples, unr_samples, pairs, suff = suff)
